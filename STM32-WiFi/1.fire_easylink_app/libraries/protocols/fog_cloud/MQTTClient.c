...
int deliverMessage(Client* c, MQTTString* topicName, MQTTMessage* message)
{
    ...
    if (rc == EC_FAILURE && c->defaultMessageHandler != NULL) 
    {
        MessageData md;
        NewMessageData(&md, topicName, message);
...
int keepalive(Client* c)
{
    ...
    if (expired(&c->ping_timer))
    {
        if (!c->ping_outstanding)
        {
            ...
            if (len > 0 && (rc = sendPacket(c, len, &timer)) == EC_SUCCESS) // send the ping packet
                c->ping_outstanding = 1;
        }
        //****************************** add wes
        // ping request has been sent, no ping response recived in keepAliveInterval, 
        // means disconnected. add by WES 20141021
        else
        {
          c->isconnected = 0;  // disconnect flag
          rc = EC_FAILURE;  //need to reconnect
          return rc;
        }
        //***************************** add wes
    }
  ...
  case PUBREC:
    {
      ...
      if (MQTTDeserialize_ack(&type, &dup, &mypacketid, c->readbuf, c->readbuf_size) != 1)
        ...
        rc = EC_FAILURE;
      else if ((rc = sendPacket(c, len, timer)) != EC_SUCCESS) // send the PUBREL packet
        rc = EC_FAILURE; // there was a problem
      //if (rc == EC_FAILURE)
      //    goto exit; // there was a problem
      break;
    }
...
int MQTTConnect(Client* c, MQTTPacket_connectData* options)
{
    ...
    if ((len = MQTTSerialize_connect(c->buf, c->buf_size, options)) <= 0) 
      goto exit;
    if ((rc = sendPacket(c, len, &connect_timer)) != EC_SUCCESS)  // send the connect packet
      goto exit; // there was a problem
...
{ 
    ...
    if (len <= 0)
        goto exit;
    if ((rc = sendPacket(c, len, &timer)) != EC_SUCCESS) // send the subscribe packet
        goto exit;             // there was a problem
...
int MQTTPublish(Client* c, const char* topicName, MQTTMessage* message)
{
    ...
    if (!c->isconnected){
         rc = EC_SOCKET_ERR;   // MQTT connect error
        goto exit;
    }
