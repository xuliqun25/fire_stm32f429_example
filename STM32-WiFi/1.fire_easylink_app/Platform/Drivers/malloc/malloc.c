...
__align(32) uint8_t mem2base[MEM2_MAX_SIZE] __attribute__((at(SDRAM_BANK_ADDR)));					//外部SDRAM内存池
//内存管理表
uint32_t mem1mapbase[MEM1_ALLOC_TABLE_SIZE];													//内部SRAM内存池MAP
uint32_t mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((at(SDRAM_BANK_ADDR+MEM2_MAX_SIZE)));	//外部SDRAM内存池MAP

#elif defined (__IAR_SYSTEMS_ICC__)     //IAR
//内存池(4字节对齐)
#pragma   pack(32)
uint8_t mem1base[MEM1_MAX_SIZE];							                //内部SRAM内存池
uint8_t mem2base[MEM2_MAX_SIZE] @SDRAM_BANK_ADDR;				            //外部SDRAM内存池
#pragma   pack()

//内存管理表
uint32_t mem1mapbase[MEM1_ALLOC_TABLE_SIZE];			                        //内部SRAM内存池MAP
uint32_t mem2mapbase[MEM2_ALLOC_TABLE_SIZE] @(SDRAM_BANK_ADDR + MEM2_MAX_SIZE);	    //外部SRAM内存池MAP
#endif
...
void my_mem_init(uint8_t memx)
{
    mymemset(mallco_dev.memmap[memx],0,memtblsize[memx]*sizeof(memtblsize[0]));	//内存状态表数据清零
...
uint32_t my_mem_malloc(uint8_t memx,uint32_t size)
{
    ...
    for(offset=memtblsize[memx]-1;offset>=0;offset--)//搜索整个内存控制区
    {
		...
		if(cmemb==nmemb)							//找到了连续nmemb个空内存块
		{
            ...
            {
                mallco_dev.memmap[memx][offset+i]=0;
...
//sdram对外的三个接口函数如下,类似于 malloc free
