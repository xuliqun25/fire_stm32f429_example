...
#define sdram_log(M, ...)   custom_log("SDRAM", M, ##__VA_ARGS__)
...
OSStatus SDRAM_Test(void)
{
  ...
  for(counter = 0; counter<IS42S16400J_SIZE;counter++ )
  {
    ...
    {
      sdram_log("8bit data read write error");
      return kGeneralErr;
    }
  }
