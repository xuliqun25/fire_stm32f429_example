...
#define ov5640_AF_log(M, ...) custom_log("ov5640_AF", M, ##__VA_ARGS__)
...
static void OV5640_FOCUS_AD5820_Init(void)
{
    ...
    ov5640_AF_log("OV5640_FOCUS_AD5820_Init\n");     

    OV5640_WriteReg(0x3000, 0x20);
    totalCnt = sizeof(OV5640_AF_FW); 
    ov5640_AF_log("Total Count = %d\n", totalCnt);
...
//    Delay(100);
    do {
        ...
        ov5640_AF_log("when init af, state=0x%x\n",state);	
	 
        mico_thread_msleep(10);
        if (iteration-- == 0)
        {
            ov5640_AF_log("[OV5640]STA_FOCUS state check ERROR!!, state=0x%x\n",state);	
            break;
        }
    } 
    while(state!=0x70);
