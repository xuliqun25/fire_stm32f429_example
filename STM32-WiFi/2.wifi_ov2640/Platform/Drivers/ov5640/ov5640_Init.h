...
OSStatus OV5640_SCCB_Init(ImageFormat_TypeDef ImageFormat);
void OV5640_QQVGAConfig(void);
void OV5640_QVGAConfig(void);
...
OSStatus HAL_DCMI_Start_DMA(uint32_t pData, uint32_t Length);
OSStatus DCMI_DMA_MultiBufferStart_IT(uint32_t SrcAddress, uint32_t DstAddress, uint32_t SecondMemAddress, uint32_t DataLength);
